...
class DistributedContext:
    ...
    def new_local_tensor(self, *args, **kwargs):
        ...
        def _remove_no_need_keys():
            # to match the input of framework create_var input
            if "rank" in kwargs:
                kwargs.pop("rank")
        ...
        assert rank in self.dist_main_programs, "The rank {} is not in dist context.".format(rank)
        ...
        program = self.dist_main_programs[rank]
        ...
        if "dist_tensor" not in kwargs:
            ...
            _remove_no_need_keys()
            local_tensor = dist_tensor.new_local_tensor(block, rank, *args, **kwargs)
