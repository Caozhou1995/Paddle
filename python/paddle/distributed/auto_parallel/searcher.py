...
def get_all_distributed_main_program(serial_program_info, dist_context):
    ...
    ranks = paddle.distributed.get_world_size() if cluster is None else len(cluster.get_all_devices("GPU"))
    for rank_id in range(ranks):
        ...
        dist_main_program, dist_startup_program = get_distributed_program(serial_program_info, used_dist_context, rank_id)
        all_dist_main_program.append(dist_main_program)

    return all_dist_main_program


def get_distributed_program(serial_program_info, dist_context, rank_id):
    ...
    opt_ops = partitioner.apply_optimize(
        copy.deepcopy(optimizer), dist_params_grads, dist_main_program, dist_startup_program)
...
class SerialProgramInfo:
    def __init__(self, train_program, satrtup_program, loss, optimizer, cluster=None):
...
class Checker:
    ...
    def check_dims_mapping_for_op(op, op_dist_attr, vars):
        ...
        for var_name in op.input_arg_names:
            ...
            if not Checker.check_dims_mapping_for_tensor(process_mesh.topology, vars[var_name].shape,
                                    dims_mapping):
                return False
...
class MCMC(SearchAlgorithm):
    ...
    def enum_valid_dist_attr_for_op(self, program, op, process_mesh):
        ...
        for composed_dims_mapping in composed_dims_mapping_list:
            ...
            if dist_op_impl_container is None:
                if is_elementwise_like_op(op.type):
                    ...
                    if valid and not changed:
                        if Checker.check_dims_mapping_for_op(op, dist_op.dist_attr, vars) and Checker.check_dims_mapping_for_special_op(op, dist_op.dist_attr, vars):
            ...
            for idx, impl in enumerate(impls):
                if impl.is_auto_compatible(dist_op):
                    if Checker.check_dims_mapping_for_op(op, dist_op.dist_attr, vars):
        ...
        if not op_valid_dist_attrs:
            ...
            dist_op = DistributedOperator(op, op_dist_attr)
            for var_name in op.input_arg_names:
                op_dist_attr.set_input_dims_mapping(vars[var_name], [-1 for i in vars[var_name].shape])
            for var_name in op.output_arg_names:
                op_dist_attr.set_output_dims_mapping(vars[var_name], [-1 for i in vars[var_name].shape])
            dist_op.dist_attr.impl_idx = -1
            op_valid_dist_attrs.append(dist_op.dist_attr)
    ...
    def search_once(self, program, valid_dist_attr_dict, dist_context, pipeline_process_meshes=None):
        ...
        ops = []
        for op in raw_ops:
            if op.type not in MCMC.not_enum_ops:
                ops.append(op)
