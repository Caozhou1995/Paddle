...
def all_params(mp, pp, sharding, h, l, V):
    P = 12 * l * h * h * (1 + 13.0 / (12 * h) + V / (12.0 * l * h))
    return P * 6 / (mp * pp) + P * 12 / (mp * pp * sharding)


def full_recompute_acts(mp, pp, s, b, h, l):
    return (2 * b * s * h / mp) * l + 2 * b * s * h / mp * pp


def all_acts(mp, pp, s, b, h, l, a):
    multiple_of = mp * a
    _h = int(8 / 3 * h)
    _h = int((_h + multiple_of - 1) / multiple_of) * multiple_of
    acts = (
        (14 * b * s * h + 4 * b * a * s + 16 * b * s) / mp
        + 4 * b * s * h
        + 8 * s * h / a
        + 8 * s * _h / mp
    )
    return acts * l + 2 * b * s * h / mp * pp
